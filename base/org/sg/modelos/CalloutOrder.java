...
public class CalloutOrder extends CalloutEngine
{
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				/*
				 * Modificado por Yamel Senih: 15/04/2011, 12:19
				 * para que aparesca la dirección Fiscal y la de Entrega
				 */
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (Env.isSOTrx(ctx, WindowNo))
		{
			...
			if (product.isStocked())
			{
				...
				/*
				 * Modificafo por: Yamel Senih 08/04/2011 17:10
				 * para que busque existencia del producto en el almacen
				 * seleccionado en la linea de la orden
				 */
				//int M_Warehouse_ID = Env.getContextAsInt(ctx, WindowNo, "M_Warehouse_ID");
				int M_Warehouse_ID = Integer.parseInt(String.valueOf(mTab.getValue("M_Warehouse_ID")));
				//Fin Modificación Yamel Senih
	...
	/**
	 * Calcula la fecha de cierre de una orden
	 * @author Yamel Senih 01/06/2011, 10:14:58
	 * @param fecha
	 * @return
	 * @return Date
	 */
	private Date getDateClose(Properties ctx, Date fecha){
		
		String sql = "SELECT Date1 FROM C_NonBusinessDay " +
			" WHERE AND AD_Client_ID = ?" +
			" AND Date1 BETWEEN ? AND ? + 30";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
		...
		for(int i = 0; i <= 3; i ++){
			...
			fechaInicio.roll(Calendar.DAY_OF_YEAR, 1);			
		}
