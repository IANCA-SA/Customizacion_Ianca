...
public class CalloutInvoice extends CalloutEngine
{
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		String sql = "SELECT p.AD_Language,p.C_PaymentTerm_ID,"
			...
			+ "lbill.C_BPartner_Location_ID Bill_Location_ID " 
			+ " FROM C_BPartner p"
			+ " INNER JOIN C_BP_Group g ON (p.C_BP_Group_ID=g.C_BP_Group_ID)"			
			+ " LEFT OUTER JOIN C_BPartner_Location l ON (p.C_BPartner_ID=l.C_BPartner_ID AND l.IsShipTo='Y' AND l.IsActive='Y')"
		...
		try
		{
			...
			if (rs.next())
			{
				...
				int bill_Location = rs.getInt("Bill_Location_ID");
				...
				if (bill_Location == 0)
					...
					mTab.setValue("Bill_Location_ID", new Integer(bill_Location));
