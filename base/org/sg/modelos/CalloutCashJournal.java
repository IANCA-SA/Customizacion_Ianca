...
public class CalloutCashJournal extends CalloutEngine
{
	...
	public String amounts (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		{	
			if(mTab.getValue("IsOverUnderPayment").equals(false) 
					|| mTab.getValue("IsOverUnderPayment").equals("N")){
	...
	/**
	 * Establece si el campo XXAfectaLibro dependiendo de si
	 * el documento es transaccion de venta o de compra
	 * @author Yamel Senih 13/04/2011, 20:31:33
	 * @param ctx
	 * @param WindowNo
	 * @param mTab
	 * @param mField
	...
	public String docType (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value){
		...
		if(docType.isSOTrx()){
			mTab.setValue("XXAfectaLibro", "N");
			mTab.setValue("C_BPartner_ID", null);
		} else {
			mTab.setValue("XXAfectaLibro", "Y");
		}
	...
	/**
	 * Coloca en Cero el monto de Ahuste
	 * @author Yamel Senih 13/04/2011, 23:00:29
	 * @param ctx
	 * @param WindowNo
	 * @param mTab
	 * @param mField
	 ...
	 * @return String
	 */
	public String WriteOff (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value){
		if (value.equals(true)){
			...
			try
			{
				...
				if (rs.next())
				{
					...
					//mTab.setValue("Amount", PayAmt.subtract(DiscountAmt));
					mTab.setValue("DiscountAmt", DiscountAmt);
					mTab.setValue("WriteOffAmt", PayAmt.subtract(DiscountAmt).subtract((BigDecimal)mTab.getValue("Amount")));					
					//Env.setContext(ctx, WindowNo, "InvTotalAmt", PayAmt.toString());
				}
			}
			catch (SQLException e)
