...
public class FinStatement extends SvrProcess
{
	...
	{
		StringBuffer sb = new StringBuffer ("SELECT ").append(getAD_PInstance_ID()).append(",0,0,")
		...
		try
		{
			...
			String sql;
			while (rs.next()){
				beginBalance = rs.getBigDecimal(9);
				sql = new String("INSERT INTO T_ReportStatement "
			+ "(AD_PInstance_ID, Fact_Acct_ID, LevelNo,"
			+ "DateAcct, Name, Description,"
			+ "AmtAcctDr, AmtAcctCr, Balance, Qty) VALUES(" +
						rs.getInt(1) + "," +
						rs.getInt(2) + "," +
						0 + "," +
						"'" + rs.getDate(4) + "'," +
	...
	private void createDetailLines()
	{
		StringBuffer sb = new StringBuffer("SELECT ")
		 	.append(getAD_PInstance_ID()).append(",Fact_Acct_ID,1,")
			...
			//Carlos Parada 29/05/2012 (Se agrego orden de socio de negocio)
			//.append(" ORDER BY DateAcct ASC");
			.append(" ORDER BY C_BPartner_ID,DateAcct ASC");
			//Fin Carlos Parada
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int no = 0;
		try
		{
			...
			int i = 1;
			while (rs.next()){
				beginBalance = beginBalance.add(rs.getBigDecimal(9));
				sql = new String("INSERT INTO T_ReportStatement "
						+ "(AD_PInstance_ID, Fact_Acct_ID, LevelNo,"
						+ "DateAcct, Name, Description,"
						+ "AmtAcctDr, AmtAcctCr, Balance, Qty) VALUES(" +
		...
		/*
		 * Modificado por Yamel Senih 29/07/2011, 13:10
		 * Se agregó la traducción
		 * */
		//	Set Name,Description
		String sql_select = "SELECT te.Name, fa.Description "
			+ "FROM Fact_Acct fa"
			+ " INNER JOIN AD_Table t ON (fa.AD_Table_ID=t.AD_Table_ID)"
			+ " INNER JOIN AD_Element e ON (t.TableName||'_ID'=e.ColumnName) "
			+ " LEFT JOIN AD_Element_Trl te ON (te.AD_Element_ID = e.AD_Element_ID) "
