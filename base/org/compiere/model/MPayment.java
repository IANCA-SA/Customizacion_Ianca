...
*/
	 ...
	 * 	Reverse New Date 
	 * 	@return true if success
	 *  reversa el documento de pago y le coloca una nueva fecha 
	 */
	public boolean reverseItNewDate(Timestamp newDate)
	{
		log.info(toString());
		// Before reverseCorrect
		m_processMsg = ModelValidationEngine.get().fireDocValidate(this,ModelValidator.TIMING_BEFORE_REVERSECORRECT);
	 ...
	 * 	Unkink Invoices and Orders and delete Allocations
	 */
	private void deAllocateNewDate(Timestamp NewDate)
	{
		if (getC_Order_ID() != 0)
			setC_Order_ID(0);
	//	if (getC_Invoice_ID() == 0)
	//		return;
		...
		for (int i = 0; i < allocations.length; i++)
		{
			allocations[i].setDateAcct(NewDate);
			allocations[i].setDateTrx(NewDate);
			allocations[i].set_TrxName(get_TrxName());
			allocations[i].setDocAction(DocAction.ACTION_Reverse_Correct);
			if (!allocations[i].processIt(DocAction.ACTION_Reverse_Correct))
				throw new AdempiereException(allocations[i].getProcessMsg());
