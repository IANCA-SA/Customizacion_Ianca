...
public class MInvoice extends X_C_Invoice implements DocAction
{
	...
	public MInvoice (MOrder order, int C_DocTypeTarget_ID, Timestamp invoiceDate)
	{
		...
		/*
		 * Se modifico la linea donde aparece:
		 * setC_BPartner_Location_ID(order.getBill_Location_ID());
		 */
		
		/*
		 * Se establece la Dirección de Entrega del Socio de Negocio
		 */
		setC_BPartner_Location_ID(order.getC_BPartner_Location_ID());
		
		setAD_User_ID(order.getBill_User_ID());	
		
		/*	Ésta linea Se agregó con la finalidad de 
		 * copiar el ID del (Lote de Contraparte)
		 * de la Orden de compra a la Factura que se creará  
		 */
		setXX_Convenio_Cosecha_ID(order.getXX_Convenio_Cosecha_ID());
		
		/*
		 * Se establece la regla de entrega 
		 */
		setDeliveryViaRule(order.getDeliveryViaRule());
		
		/*
		 * Obtiene el ID del Transportista y lo Establece en 
		 * la factura que se va a crear
		 */
		setM_Shipper_ID(order.getM_Shipper_ID());
		
		/*
		 * Se establece el Conductor
		 */
		setXX_Conductor_ID(order.getXX_Conductor_ID());
		
		/*
		 * Se establece el Vehículo
		 */
		setXX_Vehiculo_ID(order.getXX_Vehiculo_ID());
		
		/*
		 * Se establece la Dirección Fiscal del Socio de Negocio
		 */
	...
	{
		System.err.println("Me llamaron");
		log.fine("");
		//	Delete Taxes
		...
		{
			MInvoiceLine line = lines[i];
			//	Yamel Senih 18/05/2011, 14:18 
			//	No guarda los impuestos que no afectan los libros de Compra en la linea de impuesto de Orden
			MTax tax = new MTax(getCtx(), line.getC_Tax_ID(), get_TrxName());
			if(tax.isXXAfectaLibro()){	
				...
				log.info("El impuesto: " + tax.getName() + " No va pa'l Baile");
			}	//	Fin Yamel Senih
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	public boolean reverseCorrectIt()
	{
		...
		int tipoDocReverso = getC_DocTypeTarget().getXX_Doc_Reverso_ID();
		 ...
		 * @fecha 01/10/2012 
		 * coloca el documento original como descuento no aplicado
		 */
		
		if (get_Value("XX_C_Invoice_ID")!=null)
		{
			MInvoice ifac = new MInvoice(getCtx(), (Integer)get_Value("XX_C_Invoice_ID"), get_TrxName());

			ifac.set_Value("XX_DiscountApplied", "N");
			ifac.save(get_TrxName());
		}
		/**
		 * Fin Carlos Parada
		 */
