...
public final class MPaySelectionCheck extends X_C_PaySelectionCheck
{
	 ...
	 * @return int
	 */
	public static int exportToFile (MPaySelectionCheck[] checks, int p_C_BankAccount_ID, int p_C_PaySelection_ID, File file)
	{	
		int noLines = 0;
		if (checks == null || checks.length == 0)
			return 0;
		...
		for (int i = 0; i < checks.length; i++)
		{
			...
			if (check.getC_Payment_ID() != 0)
			{
				...
				if (check.getPaymentRule().equals(PAYMENTRULE_Check))
				{
					...
					/**
					 * Carlos Parada 31/05/2012 Colocar Nro de Cheque en Documento
					 * */
					payment.setDocumentNo(check.getDocumentNo());
					/**
					 * Fin Carlos Parada
					 */
