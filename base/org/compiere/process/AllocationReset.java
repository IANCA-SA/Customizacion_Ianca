...
public class AllocationReset extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		int count = 0;
		/**
		 * Carlos Parada 06/11/2012
		 * Evaluacion de Periodo Cerrado
		 */
		MAllocationHdr hdr = new MAllocationHdr(getCtx(), p_C_AllocationHdr_ID, m_trx.getTrxName());
		if (MPeriod.isOpen(getCtx(), hdr.getDateAcct(), MDocType.DOCBASETYPE_PaymentAllocation,hdr.getAD_Org_ID()))
		{
			...
			try
			{
				...
				while (rs.next ())
				{
					hdr = new MAllocationHdr(getCtx(), rs, m_trx.getTrxName());
					if (delete(hdr))
						count++;
				}
	 		}
		...
		else
		{
			try
			{
				MAllocationHdr hdrcpy = new MAllocationHdr(getCtx(), 0, m_trx.getTrxName());

				//hdrcpy.setDescription(hdr.getDescription()+ " -> " + hdr.getDocumentNo() );
				hdrcpy.addDescription("{->" + hdr.getDocumentNo() + ")");
				hdrcpy.setC_Currency_ID(hdr.getC_Currency_ID());
				hdrcpy.saveEx(m_trx.getTrxName()); 
				DocumentNo =hdrcpy.getDocumentNo();
				
				hdr.addDescription("(" + DocumentNo + "<-)");
								
				//hdr.setDescription(hdr.getDescription() + " <- " + hdrcpy.getDocumentNo());
				hdr.saveEx(m_trx.getTrxName());
				MAllocationLine[] malls = hdr.getLines(false);
				
				for (MAllocationLine mall:malls)
				{
					MAllocationLine mallcpy =new MAllocationLine(getCtx(), 0, m_trx.getTrxName());
					mall.copyValues(mall, mallcpy);
					mallcpy.setC_AllocationHdr_ID(hdrcpy.getC_AllocationHdr_ID());
					mallcpy.setAmount(mallcpy.getAmount().negate());
					mallcpy.setDiscountAmt(mallcpy.getDiscountAmt().negate());
					mallcpy.setWriteOffAmt(mallcpy.getWriteOffAmt().negate());
					mallcpy.saveEx(m_trx.getTrxName());
					mallcpy.reverseIt(true);
				}
				hdrcpy.completeIt();
				m_trx.commit();
			}
