...
public class ImportProduct extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		//	Added By Jorge Colmenarez 2015-12-01
		//	Add Support for Sub Category Product 
		//	Set Product Sub Category
		sql = new StringBuffer ("UPDATE I_Product i "
			+ "SET YY_Sub_Categoria_ID=(SELECT MAX(YY_Sub_Categoria_ID) FROM YY_Sub_Categoria sc "
			+ "WHERE i.SubCategoryName=sc.YY_Nombre AND i.AD_Client_ID=sc.AD_Client_ID) "
			+ "WHERE SubCategoryName IS NOT NULL AND YY_Sub_Categoria_ID IS NULL"
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.info("Set Sub Category=" + no);
		//	Set Product Category By Sub Category
		sql = new StringBuffer ("UPDATE I_Product i "
			+ "SET M_Product_Category_ID=(SELECT MAX(M_Product_Category_ID) FROM YY_Sub_Categoria sc "
			+ "WHERE i.YY_Sub_Categoria_ID=sc.YY_Sub_Categoria_ID AND i.AD_Client_ID=sc.AD_Client_ID) "
			+ "WHERE SubCategoryName IS NOT NULL AND M_Product_Category_ID IS NULL"
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		log.info("Set Product Category=" + no);
		//	Invalid Sub Category
		sql = new StringBuffer ("UPDATE I_Product "
			+ "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Sub-Category,' "
			+ "WHERE SubCategoryName IS NOT NULL AND YY_Sub_Categoria_ID IS NULL"
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		if (no != 0)
			log.warning("Invalid Sub-Category=" + no);
		//	Set Tax Category
		sql = new StringBuffer ("UPDATE I_Product i "
			+ "SET C_TaxCategory_ID=(SELECT MAX(C_TaxCategory_ID) FROM C_TaxCategory tc "
			+ "WHERE i.NameTaxCategory=tc.Name AND i.AD_Client_ID=tc.AD_Client_ID) "
			+ "WHERE NameTaxCategory IS NOT NULL AND C_TaxCategory_ID IS NULL"
			+ " AND I_IsImported<>'Y'").append(clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
...
*/
			...
			while (rs.next())
			{
				...
				if (newProduct)			//	Insert new Product
				{
					...
					//	Added By Jorge Colmenarez 2015-12-01
					//	Add Support for Sub Category, Tax Category, Purchase, Sold, Stocked
					product.set_CustomColumn("YY_Sub_Categoria_ID",imp.get_ValueAsInt("YY_Sub_Categoria_ID"));
					...
					if(imp.get_ValueAsInt("C_TaxCategory_ID")==0)
						product.setC_TaxCategory_ID(imp.get_ValueAsInt("C_TaxCategory_ID"));
					else
				...
				{
					//	Added By Jorge Colmenarez 2015-12-01 
					//	Add Support for Update Product Fields Sub Category, Tax Category, Purchase, Sold, Stocked
					//	Comment Old Query
					/*
					String sqlt = "UPDATE M_PRODUCT "
						+ "SET (Value,Name,Description,DocumentNote,Help,"
					...
					*/
					...
					String sqlt = "UPDATE M_PRODUCT "
							...
							+ "Volume,Weight,ShelfWidth,ShelfHeight,ShelfDepth,UnitsPerPallet,"
							+ "Discontinued,DiscontinuedBy, DiscontinuedAt, Updated,UpdatedBy,"
							+ "YY_Sub_Categoria_ID,C_TaxCategory_ID,IsPurchased,IsSold,IsStocked)= "
							+ "(SELECT Value,Name,Description,DocumentNote,Help,"
							+ "UPC,SKU,C_UOM_ID,M_Product_Category_ID,Classification,ProductType,"
							+ "Volume,Weight,ShelfWidth,ShelfHeight,ShelfDepth,UnitsPerPallet,"
							+ "Discontinued,DiscontinuedBy, DiscontinuedAt, SysDate,UpdatedBy,"
