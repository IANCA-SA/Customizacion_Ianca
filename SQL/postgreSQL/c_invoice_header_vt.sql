...
CREATE OR REPLACE VIEW c_invoice_header_vt AS 
 SELECT i.ad_client_id, i.ad_org_id, i.isactive, i.created, i.createdby, i.updated, i.updatedby, dt.ad_language, i.c_invoice_id, i.issotrx, i.documentno, i.docstatus, i.c_doctype_id, i.c_bpartner_id, bp.value AS bpvalue, bp.taxid AS bptaxid, bp.naics, bp.duns, oi.c_location_id AS org_location_id, oi.taxid, dt.printname AS documenttype, dt.documentnote AS documenttypenote, i.c_order_id, i.salesrep_id, COALESCE(ubp.name, u.name) AS salesrep_name, i.dateinvoiced, bpg.greeting AS bpgreeting, bp.name, bp.name2, bpcg.greeting AS bpcontactgreeting, bpc.title, bpl.phone, NULLIF(bpc.name::text, bp.name::text) AS contactname, bpl.c_location_id, bp.referenceno, l.postal::text || l.postal_add::text AS postal, i.description, i.poreference, i.dateordered, i.c_currency_id, pt.name AS paymentterm, pt.documentnote AS paymenttermnote, i.c_charge_id, i.chargeamt, i.totallines, i.grandtotal, i.grandtotal AS amtinwords, i.m_pricelist_id, i.istaxincluded, i.c_campaign_id, i.c_project_id, i.c_activity_id, i.ispaid, COALESCE(oi.logo_id, ci.logo_id) AS logo_id, i.bill_location_id, cc.xx_convenio_cosecha_id, tr.m_shipper_id, con.xx_conductor_id, veh.xx_vehiculo_id, i.xx_nrocontrol, sum(
        CASE
        ...
        CASE
            WHEN if.taxamt <> 0::numeric THEN if.taxbaseamt
            ELSE 0::numeric
        END) AS totalbi, o.documentno AS nro_doc_orden,
  ...
  i.updatedby, i.c_invoice_id, i.issotrx, i.documentno, i.docstatus, i.c_doctype_id, i.c_bpartner_id, 
  bp.value, bp.taxid, bp.naics, bp.duns, oi.c_location_id, oi.taxid, dt.printname, dt.documentnote, 
  i.c_order_id, i.salesrep_id, i.dateinvoiced, bpg.greeting, bp.name, bp.name2, bpcg.greeting, 
  ...
  i.totallines, i.grandtotal, i.m_pricelist_id, i.istaxincluded, i.c_campaign_id, i.c_project_id, 
  i.c_activity_id, i.ispaid, i.bill_location_id, cc.xx_convenio_cosecha_id, tr.m_shipper_id, 
  con.xx_conductor_id, veh.xx_vehiculo_id, i.xx_nrocontrol, ubp.name, u.name, bpc.name, l.postal, 
  l.postal_add, oi.logo_id, ci.logo_id, dt.ad_language, o.documentno, 
